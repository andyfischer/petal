...
| Capability | Status | Notes |
|---|---|---|
| Incremental dataflow update (recompute only affected nodes) | 🟡 | Two layers shipped: every host gates frames on what the last run actually read ([frame-gate.md](frame-gate.md)), so a quiet frame costs nothing; and within a frame that runs, every user-function call is a memoized scope replayed from its record when its arguments, captures and recorded reads are unchanged ([memo-scopes.md](memo-scopes.md)) — a pointer move re-runs the rows it crossed, not the list. What remains: the top-level script body is not a scope, a list still pays one validation per row, and a data change still walks the whole collection. Dependency classes (guarding whole blocks by what can reach them), keyed collections and a hit-test index are the next steps, and the ceiling on app size until then. Roadmap and status: [reactive-rendering-plan.md](reactive-rendering-plan.md). |
