...
The bytecode VM is feature-complete and default-on: it runs the entire language
at full behavioral parity with the graph engine (value, print output, final
state, and error text across the whole `examples/` corpus and the vitest suite),
is 4–10x faster on compute-bound code, and — by default — mutates provably-unique
...
profile does not cover — the standing rule is **re-run `ts/bin/bench-backends.ts`
...
- **The four oracles** — graph / BC-noopt / BC-route-A-only / BC-all — must agree
  exactly (value, output, state, error text). The graph engine is canonical.
...
correctness surface against the four oracles) for an unquantified win, and the VM
is already 4–10x faster than the reference. The packed encoding was always
...
The graph engine **re-annotates** an already-annotated error as it propagates
back up through a synchronous intrinsic call (`call_closure_sync`); the VM
matches this by annotating at every `step()` error. During the parity push this
was deliberately preserved (don't "fix" the apparent double-annotation while
parity is the goal).

Now that bytecode is the default, the open question stands: **is this the
semantics or a quirk?** If a quirk, fix it in *both* engines at once via the
shared `backend/errors.rs`, re-soak the fuzzer (error text is part of the
