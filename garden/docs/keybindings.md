...
| Scroll | `Ctrl+D` `Ctrl+U` `Ctrl+F` `Ctrl+B`, `Ctrl+E` `Ctrl+Y`, `zt` `zz` `zb` | `Ctrl+D`/`Ctrl+U` move the cursor down / up half a viewport and `Ctrl+F`/`Ctrl+B` a whole one, clamped to the buffer ends and extending the selection in Visual mode; `Ctrl+E`/`Ctrl+Y` roll the *view* down / up one line (count-aware, `3<C-e>`) without moving the cursor, except along the edge it would otherwise scroll past; `zt`/`zz`/`zb` reposition the viewport so the cursor's line sits at the top / center / bottom (cursor stays put). All of them work inside a focused **panel region** too — the `garden diff` / `garden pr` review panes. |
