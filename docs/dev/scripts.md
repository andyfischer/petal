...
| `./ts/bin/native-effect-audit.ts` | Static approximation of `--effect-audit` over the source of every registration site, including natives no corpus app calls: which declare what they do, and which reach host state without declaring. Exits non-zero if any does. `--all` lists every native, `--json` for machine-readable output. See [declarative-effect-refactoring.md](../tasks/declarative-effect-refactoring.md). |
