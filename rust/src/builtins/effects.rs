...
builtin_effects![
    ...
    ("last", NO_REF | PURE),
    // `prepend`/`concat` both build a brand-new container and never reuse an
    // argument's store — unlike `append`, which is the in-place candidate.
    // (`sort_by` and the comparator form of `sort` are deliberately absent, for
    // the same reason `map`/`filter` are: they invoke user code, so no effect
    // property of theirs is knowable from the name.)
    ("prepend", FRESH | NO_REF | PURE),
    ("concat", FRESH | NO_REF | PURE),
    // character-indexed strings: each builds a brand-new string or list, and
    // strings are immutable, so nothing aliases the argument afterwards.
    ("chars", FRESH | NO_REF | PURE),
    ("char_len", NO_REF | PURE),
    ("char_at", NO_REF | PURE),
    ("char_slice", NO_REF | PURE),
    ("index_of", NO_REF | PURE),
    ...
    ("float", PURE),
    ("int", PURE),
    ("parse_float", NO_REF | PURE),
    ("parse_int", NO_REF | PURE),
    ("json_stringify", NO_REF | PURE),
    ("json_parse", FRESH | NO_REF | PURE),
    ("min", PURE),
    ("max", PURE),
    ("round", PURE),
    ("sin", PURE),
    ...
    ("minimum", PURE),
    ("maximum", PURE),
    // formatting: every one renders its arguments into a brand-new string and
    // keeps nothing.
    ("fixed", NO_REF | PURE),
    ("commas", NO_REF | PURE),
    ("pad_start", NO_REF | PURE),
    ("pad_end", NO_REF | PURE),
    ("format", NO_REF | PURE),
    // `safe_div` can broadcast over a list argument, so it is pure but claims
    // neither FRESH nor NO_REF (the conservative answer for both).
    ("safe_div", PURE),
