...
**Status.** Every harness below has verified except `range_len_is_exact` and
`int_mod_is_the_remainder`, which had not finished after 30 minutes on their
first runs; both are division-bound. The fixes they cover are also pinned by
the regression tests. If either stays slow, restate it without division, the
way `int_div_is_exact` and `range_nth_is_exact` are.
...
| `int_mod_is_the_remainder` | `%` returns the machine's truncated remainder for every nonzero divisor and never overflows (`i64::MIN % -1` is 0). The remainder's value is trusted to Rust's `%`: re-deriving it in the spec needs a second 64-bit division, which takes the solver over an hour. |
...
| `range_len_is_exact`, `range_nth_is_exact` | `range(start, end, step)` has exactly the right length for every input (up to `2^64 - 1`) and every element is computed without wraparound. |
...
| `i64::MIN % -1` reports overflow | Kani `int_mod_is_the_remainder` | "Integer overflow" | `0` |
| `random_int(lo, hi)` panics for wide ranges | Kani `scale_unit_to_range_stays_in_range` | `hi - lo` overflows | Proven in `[lo, hi)` |
| `range(n)` for huge `n` aborts the process | Kani `range_len_is_exact` (the length is exact, so the reservation can be refused) | Capacity-overflow abort, or an unbounded loop for `step > 1` | Runtime error |
...
cargo kani --harness proofs::numeric::int_mod_is_the_remainder  # one harness
